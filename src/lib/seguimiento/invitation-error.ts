export function invitationError(error: {code?: string; status?: number}): string {
 ...
 switch(error.code){
  ...
  case 'over_request_rate_limit': return pending+'Se alcanzó el límite de envíos o solicitudes de Supabase. Reintenta más tarde; el tiempo de espera depende del límite configurado.';
  case 'email_exists':
  ...
  default:
   if(error.status===429)return pending+'Se alcanzó el límite de envíos o solicitudes de Supabase. Reintenta más tarde; el tiempo de espera depende del límite configurado.';
